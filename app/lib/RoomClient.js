...
export default class RoomClient
{
	...
	)
	{
		...
		// this._protooUrl = `wss://v3demo.mediasoup.org:4443/?roomId=${roomId}&peerId=${peerId}`;
	...
	async join()
	{
		...
		console.log(this._protooUrl, 'sdfsdfsavsdabdfbdf');
				...
				case 'newDataConsumer':
				{
					...
					catch (error)
					{
						...
						store.dispatch(requestActions.notify(
							{
								type : 'error',
								text : `Error while entering class`
							}));
			...
			switch (notification.method)
			{
				...
				case 'newPeer':
				{
					...
					store.dispatch(requestActions.notify(
						{
							title: 'Class',
							text : `${peer.displayName} has joined the class`
						}));
	...
	async disableShare()
	{
		...
		catch (error)
		{
			store.dispatch(requestActions.notify(
				{
					type : 'error',
					text : `Error closing server-side share Producer`
				}));
		}
	...
	async applyNetworkThrottle({ uplink, downlink, rtt, secret })
	{
		...
		catch (error)
		{
			...
			store.dispatch(requestActions.notify(
				{
					type : 'error',
					text : `Error applying network throttle`
				}));
		}
	}
		...
		catch (error)
		{
			if (!silent)
			{
				...
				store.dispatch(requestActions.notify(
					{
						type : 'error',
						text : `Error resetting network throttle`
					}));
			}
		}
	}
	...
	async _joinRoom()
	{
		...
		catch (error)
		{
			...
			store.dispatch(requestActions.notify(
				{
					type : 'error',
					text : `Could not join class. Maybe the device and browser combination is not supported yet. Please open the app in your default mobile browser.`
				}));
